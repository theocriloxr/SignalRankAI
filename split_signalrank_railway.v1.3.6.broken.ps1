# SignalRankAI Railway split tool hotfix 1.3.6.1 (application release remains 1.3.6)
[CmdletBinding()]
param(
    ...
    [int]$ServiceCreateRetries = 15,
    [int]$ServiceCreateRetryDelaySeconds = 2
)
...
function Test-ServiceExists {
    ...
    # Windows PowerShell converts native stderr into a NativeCommandError when
    # ErrorActionPreference=Stop. A missing service is an expected probe result,
    # so suppress that one command without weakening error handling elsewhere.
    $previousErrorActionPreference = $ErrorActionPreference
    $exitCode = 1
    try {
        $ErrorActionPreference = "SilentlyContinue"
        & railway service status --service $Name --environment $Environment --json 1>$null 2>$null
...
function Wait-ServiceExists {
    ...
    for ($attempt = 1; $attempt -le $ServiceCreateRetries; $attempt++) {
        if (Test-ServiceExists -Name $Name) {
            return
        }
        if ($attempt -lt $ServiceCreateRetries) {
            Write-Host "Waiting for Railway to register service $Name ($attempt/$ServiceCreateRetries)..." -ForegroundColor DarkGray
            Start-Sleep -Seconds $ServiceCreateRetryDelaySeconds
        }
    }

    throw "Railway created request for '$Name' but the service was not visible after $ServiceCreateRetries checks. Run 'railway service status --all --environment $Environment' and rerun this script."
}
...
function Ensure-Service {
    ...
    Write-Host "Creating Railway service: $Name" -ForegroundColor Cyan
    Invoke-Railway -Arguments @("add", "--service", $Name, "--environment", $Environment, "--json") | Out-Null
    Wait-ServiceExists -Name $Name
    Write-Host "Created Railway service: $Name" -ForegroundColor Green
}
...
function Prepare-NeutralRailwayConfig {
    ...
    if (Test-Path $path) {
        ...
        if (-not (Test-Path $backup)) {
            ...
            Write-Host "Preserving existing railway.monolith.backup.json (resume-safe)." -ForegroundColor Yellow
        }
    }
