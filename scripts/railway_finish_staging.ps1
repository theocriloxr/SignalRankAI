...
function Get-DatabaseIdentityLocal {
    ...
    $raw = Invoke-WithTemporaryEnvironment -Values $envValues -Action {
        $identityOutput = & python scripts/database_identity.py --expect-head 0040_cross_channel_paper_receipt 2>&1 | Out-String
        if ($LASTEXITCODE -ne 0) {
...
function Test-ServiceLogs {
    ...
    if (-not $logs.Contains('alembic_current=0040_cross_channel_paper_receipt')) {
        throw "$Service did not prove alembic_current=0040_cross_channel_paper_receipt in its latest logs"
    }
...
$commonVariables = @(
    ...
    "APP_VERSION=1.5.1",
    "EXPECTED_ALEMBIC_HEAD=0040_cross_channel_paper_receipt",
...
Write-Host "Migration database is at 0040_cross_channel_paper_receipt fingerprint=$($databaseIdentity.fingerprint)." -ForegroundColor Green
...
$summary = [ordered]@{
    ...
    alembic_head = "0040_cross_channel_paper_receipt"
