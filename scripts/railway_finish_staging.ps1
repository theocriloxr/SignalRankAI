...
function Get-DatabaseIdentityLocal {
    ...
    $raw = Invoke-WithTemporaryEnvironment -Values $envValues -Action {
        $identityOutput = & python scripts/database_identity.py --expect-head 0039_web_signup_acquisition 2>&1 | Out-String
        if ($LASTEXITCODE -ne 0) {
...
function Test-ServiceLogs {
    ...
    if (-not $logs.Contains('alembic_current=0039_web_signup_acquisition')) {
        throw "$Service did not prove alembic_current=0039_web_signup_acquisition in its latest logs"
    }
...
$commonVariables = @(
    ...
    "APP_VERSION=1.5.1",
    "EXPECTED_ALEMBIC_HEAD=0039_web_signup_acquisition",
...
Write-Host "Migration database is at 0039_web_signup_acquisition fingerprint=$($databaseIdentity.fingerprint)." -ForegroundColor Green
...
$summary = [ordered]@{
    ...
    alembic_head = "0039_web_signup_acquisition"
