...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| IMP-001 | Wire decision intelligence through full lifecycle | AI/Architecture | High | Active engine decision paths preserve exact raw decision/reason and emit structured decision intelligence with normalized lifecycle disposition for issued, rejected, skipped, delayed and suppressed semantics. | `engine/core.py`, `services/decision_intelligence.py` | Keep richer subsystem payloads additive; do not create a second production decision path. | `tests/test_decision_intelligence.py`; clean-room commit `8766ceae...` with 356 targeted tests PASS. | Complete | 2026-06-29 | Engineering |
...
| IMP-006 | Live-calibrate trading ecosystem intelligence | Trading intelligence | High | Market intelligence, MTF classification, opportunity scoring, and mission-control recommendations are now implemented with deterministic production-safe defaults, but their weights need live replay/shadow calibration. | `services/market_intelligence.py`, `services/mtf_consensus.py`, `services/opportunity_engine.py`, `services/mission_control.py`, `engine/core.py` | Use Railway outcomes, replay, and shadow evaluation to calibrate time-to-target probabilities, confidence decay, early-exit thresholds, and strategy/session weights. | `tests/test_trading_ecosystem_intelligence.py`; post-deploy outcome coverage and replay reports. | In Progress | 2026-07-02 | Quant/Engineering |
