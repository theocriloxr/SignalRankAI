...
class Signal(Base):
    ...
    ml_recovery_mode: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False, index=True)
    ml_recovery_reason: Mapped[Optional[str]] = mapped_column(String(64))
    ml_recovery_champion_raw_probability: Mapped[Optional[float]] = mapped_column(Float)
    ml_recovery_certified_threshold: Mapped[Optional[float]] = mapped_column(Float)
    ml_recovery_challenger_probability: Mapped[Optional[float]] = mapped_column(Float)
    ml_recovery_challenger_threshold: Mapped[Optional[float]] = mapped_column(Float)
    ml_recovery_challenger_version: Mapped[Optional[str]] = mapped_column(String(64))
...
class PaperTradeAttempt(Base):
    ...
    signal_id: Mapped[str] = mapped_column(String(36), ForeignKey("signals.signal_id"), index=True, nullable=False)
    delivery_id: Mapped[Optional[int]] = mapped_column(ForeignKey("signal_deliveries.id"), index=True)
    receipt_channel: Mapped[str] = mapped_column(String(16), default="telegram", nullable=False)
    receipt_reference: Mapped[Optional[str]] = mapped_column(String(64), index=True)
    idempotency_key: Mapped[str] = mapped_column(String(128), nullable=False)
