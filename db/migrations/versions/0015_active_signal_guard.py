"""Add active-signal uniqueness and hot-path indexes.

Revision ID: 0015_active_signal_guard
Revises: 0014_add_outcome_pnl_pct
Create Date: 2026-06-22

Legacy databases can contain more than one ``status='active'`` row for the
same asset/direction/timeframe.  PostgreSQL cannot create the partial unique
index until those rows are reconciled.  This migration therefore preserves all
signal rows and their delivery/outcome history, selects one deterministic
canonical active row, marks only the other duplicate rows as ``superseded``,
and then creates the index idempotently.
"""
from __future__ import annotations

import logging

from alembic import op
import sqlalchemy as sa


revision = "0015_active_signal_guard"
down_revision = "0014_add_outcome_pnl_pct"
branch_labels = None
depends_on = None

logger = logging.getLogger("alembic.runtime.migration")


_POSTGRES_RECONCILE_SQL = """
WITH active_candidates AS (
    SELECT
        s.signal_id,
        s.asset,
        s.direction,
        s.timeframe,
        s.created_at,
        CASE WHEN EXISTS (
            SELECT 1
            FROM outcomes o
            WHERE o.signal_id = s.signal_id
              AND LOWER(COALESCE(o.status, '')) IN ('active', 'tp1', 'tp2')
        ) THEN 1 ELSE 0 END AS has_open_outcome,
        COALESCE(deliveries.delivery_count, 0) AS delivery_count
    FROM signals s
    LEFT JOIN (
        SELECT signal_id, COUNT(*)::bigint AS delivery_count
        FROM signal_deliveries
        GROUP BY signal_id
    ) deliveries ON deliveries.signal_id = s.signal_id
    WHERE s.status = 'active'
), ranked AS (
    SELECT
        signal_id,
        asset,
        direction,
        timeframe,
        has_open_outcome,
        delivery_count,
        ROW_NUMBER() OVER (
            PARTITION BY asset, direction, timeframe
            ORDER BY
                has_open_outcome DESC,
                delivery_count DESC,
                created_at DESC NULLS LAST,
                signal_id DESC
        ) AS row_num,
        FIRST_VALUE(signal_id) OVER (
            PARTITION BY asset, direction, timeframe
            ORDER BY
                has_open_outcome DESC,
                delivery_count DESC,
                created_at DESC NULLS LAST,
                signal_id DESC
        ) AS canonical_signal_id
    FROM active_candidates
), duplicates AS (
    SELECT * FROM ranked WHERE row_num > 1
), audit AS (
    INSERT INTO admin_events (
        event_type,
        actor_telegram_user_id,
        details,
        created_at
    )
    SELECT
        'migration_active_signal_dedupe',
        NULL,
        json_build_object(
            'revision', '0015_active_signal_guard',
            'superseded_signal_id', d.signal_id,
            'canonical_signal_id', d.canonical_signal_id,
            'asset', d.asset,
            'direction', d.direction,
            'timeframe', d.timeframe,
            'had_open_outcome', (d.has_open_outcome = 1),
            'delivery_count', d.delivery_count,
            'action', 'status_changed_to_superseded',
            'rows_deleted', 0
        ),
        NOW()
    FROM duplicates d
    RETURNING 1
)
UPDATE signals s
SET status = 'superseded'
FROM duplicates d
WHERE s.signal_id = d.signal_id
RETURNING s.signal_id
"""


_SQLITE_RECONCILE_SQL = """
WITH ranked AS (
    SELECT
        signal_id,
        ROW_NUMBER() OVER (
            PARTITION BY asset, direction, timeframe
            ORDER BY created_at DESC, signal_id DESC
        ) AS row_num
    FROM signals
    WHERE status = 'active'
)
UPDATE signals
SET status = 'superseded'
WHERE signal_id IN (SELECT signal_id FROM ranked WHERE row_num > 1)
"""


def _reconcile_active_duplicates() -> int:
    """Return the number of legacy active rows changed to ``superseded``."""
    bind = op.get_bind()
    dialect = str(bind.dialect.name or "").lower()

    if dialect == "postgresql":
        # Serialize the reconciliation/index creation if two pre-deploy jobs are
        # accidentally started against the same database.
        bind.execute(
            sa.text(
                "SELECT pg_advisory_xact_lock(hashtext('signalrank:active_signal_guard'))"
            )
        )
        result = bind.execute(sa.text(_POSTGRES_RECONCILE_SQL))
    else:
        result = bind.execute(sa.text(_SQLITE_RECONCILE_SQL))

    rows = result.fetchall() if getattr(result, "returns_rows", False) else []
    changed = len(rows)
    if changed:
        logger.warning(
            "Reconciled %s duplicate active signal row(s) without deleting signal, delivery, or outcome history.",
            changed,
        )
    else:
        logger.info("No duplicate active signal rows required reconciliation.")
    return changed


def _create_indexes() -> None:
    # Raw IF NOT EXISTS DDL makes the migration safe after a manually repaired
    # or partially prepared database.  The enclosing Alembic transaction still
    # guarantees atomicity on PostgreSQL.
    op.execute(
        """
        CREATE UNIQUE INDEX IF NOT EXISTS ix_signals_active_thesis
        ON signals (asset, direction, timeframe)
        WHERE status = 'active'
        """
    )
    op.execute(
        """
        CREATE INDEX IF NOT EXISTS ix_signals_asset_status_created_at
        ON signals (asset, status, created_at)
        """
    )
    op.execute(
        """
        CREATE INDEX IF NOT EXISTS ix_outcomes_signal_status_closed_at
        ON outcomes (signal_id, status, closed_at)
        """
    )
    # ``sent_ok`` is introduced by revision 0017.  Creating its hot-path
    # index here would make a clean 0014 -> 0015 upgrade fail.  Revision 0022
    # creates the index after the proof columns exist.


def upgrade() -> None:
    _reconcile_active_duplicates()
    _create_indexes()


def downgrade() -> None:
    op.execute("DROP INDEX IF EXISTS ix_outcomes_signal_status_closed_at")
    op.execute("DROP INDEX IF EXISTS ix_signals_asset_status_created_at")
    op.execute("DROP INDEX IF EXISTS ix_signals_active_thesis")
