...
def test_v125_profile_contains_guarded_live_paystack_settings():
    root = Path(__file__).resolve().parents[1]
    profile = (root / "SignalRankAI_v1.2.5_Railway_Full_System_Live_Paystack_Staging.env.example").read_text(encoding="utf-8")
    for line in (
        "APP_VERSION=1.2.5",
...
def test_version_fingerprint_is_v125():
    ...
    assert RELEASE_FINGERPRINT == "v1.2.5-live-paystack-delivery-adaptive-telemetry-20260729"
