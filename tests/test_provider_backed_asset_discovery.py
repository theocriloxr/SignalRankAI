...
def test_twelvedata_timeseries_verifies_configured_indices(monkeypatch) -> None:
    _reset_discovery_state()
    monkeypatch.setenv("TWELVEDATA_API_KEY", "test-key")
    monkeypatch.setenv("INDEX_TICKERS", "US500,NAS100,GER40")
    monkeypatch.setattr(discovery, "_metaapi_symbols", lambda: [])
    ...
    def fake_get(url, **kwargs):
        ...
        return SimpleNamespace(
            ...
            json=lambda: {
                "status": "ok",
                "values": [{"datetime": "2026-09-23 00:00:00", "close": "100"}],
            },
        )

    monkeypatch.setattr(discovery.requests, "get", fake_get)

    indices = discovery.get_trending_index_tickers(top_n=3)
    assert set(indices) == {"US500", "NAS100", "GER40"}
    assert set(calls) == {"SPX", "NDX", "DAX"}
    for symbol in indices:
        assert "twelvedata" in discovery.asset_discovery_provenance(symbol)
